...
usage() {
    cat <<'EOF'
Usage: tools/run_m5_acceptance.sh --output <directory> [--network-runner auto|sandbox-exec]
EOF
}
...
run_suite() {
    ...
    run_offline_logged "verify-$prefix" /dev/null "$binary" verify --config "$config"
