...
usage() {
    cat <<'EOF'
Usage: tools/acceptance/run_m3_acceptance.sh --output <directory> [--network-runner auto|sandbox-exec|docker]
...
run_logged fixture-integrity "$root/tools/acceptance/verify_m3_fixtures.sh"
...
run_offline_logged verify /dev/null "$binary" data verify --config "$three_config"
...
run_offline_logged full-verify /dev/null "$binary" data verify --config "$full_config"
...
approver: tools/acceptance/run_m3_acceptance.sh
EOF
