...
use market_types::{
    ...
    QuantityUnit, QuoteSnapshot, SourceFormatId, TpexQuoteAnnotations, TradeBatch,
    TradeBatchOrdering, TradeObservationKind, TradingDate, UnknownValue, VolatilityDirection,
    Volume,
};
...
pub const MAPPING_VERSION: u16 = 7;
    ...
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::WrongMarket(market) => {
                write!(formatter, "TPEx normalizer cannot use market {market:?}")
            }
...
impl TpexNormalizer {
    ...
    fn parse_record(
        ...
        validate_identity(record_number, &context, "market", "tpex", envelope.market)?;
        validate_identity(
            record_number,
            &context,
            "symbol",
        ...
        let annotations = TpexQuoteAnnotations::new(wire.status_flags, wire.limit_flags);
    ...
    fn normalize_intermediate_group(
        ...
        if intermediates.is_empty() || finals.len() != 1 {
    ...
    fn auction_phase(
        ...
        let opening = status.delayed_open()
            || status.opening_marker()
            || (status.trial() && in_opening_window);
        let closing = status.delayed_close()
    ...
    fn session_time(&self, time: &str) -> MatchTime {
        ...
        MatchTime::parse(&value).expect("TPEx session constants are valid timestamps")
    }
...
impl fmt::Display for NormalizationError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            formatter,
            "TPEx normalization failed at input record {}",
            self.record_number
        )?;
...
fn volatility_observation(annotations: TpexQuoteAnnotations) -> AuctionObservation {
