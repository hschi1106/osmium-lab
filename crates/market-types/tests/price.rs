...
#[test]
fn price_preserves_signed_and_zero_values_exactly() {
    ...
    assert_eq!(
        Price::parse("-1").unwrap().as_decimal(),
        Decimal::parse("-1").unwrap()
    );
    assert_eq!(Price::parse("0").unwrap(), Price::new(Decimal::ZERO));
    assert_eq!(Price::positive(Decimal::ZERO), Err(PriceError::NonPositive));
    assert_eq!(Price::parse_positive("-1"), Err(PriceError::NonPositive));
    assert_eq!(
