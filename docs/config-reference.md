...
外部 Rust binary 可使用 `osmium_cli::run_with_registry_provider` 注入額外 compiled factories；lab 仍負責所有 command、錯誤分類與執行流程。strategy-specific 商業政策（例如借貸比例、利率或轉換時間）必須是 strategy parameters，不得加入通用 `simulation` schema。策略若需扣除非 fill 成本，應輸出通用 `CashChargeRequest`。
