...
本文件定義 Teralion REST API 與本地 source repository 之間的 adapter contract。Teralion 是目前的
一個 adapter，不是 core dependency；wire response 只存在 adapter／normalizer boundary，replay、
MarketState、strategy、execution 與 accounting 都只看 provider-neutral types。
...
`/ticks` 保留 source format、status flags 與試算 record；衍生的 `/trades`、`/bars`、`/quotes`
不能取代 raw timeline input。TWSE／TPEx 的 trial bit只證明來源標示為 trial，不單獨證明盤中
stability record 的每個 deal/book/cumulative-volume field；normalizer 必須結合該 market wire
contract 與完整 record shape驗證。TAIFEX 則以 `I022`／`I082` 等獨立 format分類。

Daily instrument response 保存 symbol、market、kind、underlying、option side、strike、expiry、
multiplier、currency、trading date 與 session reference 等可用欄位。`null`、空字串與 `0` 是不同值；
缺少 metadata 不得自行推定。
...
Market-specific evidence 見 [TWSE](twse.md)、[TPEx](tpex.md)與 [TAIFEX](taifex.md)。官方參考：
[Teralion Feed Archive API](https://docs.teraliontech.com/feed-archive/)。本地生命週期見
[資料流程與儲存](../architecture/data-flow.md)。
