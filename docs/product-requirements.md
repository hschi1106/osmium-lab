...
-> QuoteSnapshot / TradeBatch / IndicativeOpeningAuction / IndicativeClosingAuction
-> Market State
-> match_time replay
-> Example Strategy
...
M1 fixture 必須涵蓋 `STOCK_SNAPSHOT` 與 `STOCK_REALTIME`。non-trial final quote 以
`QuoteSnapshot` 表達；realtime intermediate print 以 `TradeBatch` 表達；Bit 7
試算揭示以 `IndicativeOpeningAuction`／`IndicativeClosingAuction` 表達，並驗證
