...
- 完成 market-types／stability migration 與 Alpha 正式 dependency pin。移除一次性計畫與
  semantic fixture exporter；完整 migration 材料保留於 Git revision `4ec928f`，驗收摘要見
  [驗證文件](docs/operations/validation.md)。
...
- `MatchTime` formatting 與 canonical event timestamp、`TradingDate` epoch constructor及 canonical vector decoding新增邊界驗證；
  `Decimal` 提供不經浮點數的 exact text formatting。
...
- 移除 `InstrumentClass::Unknown` 與依 market／symbol 推定商品類型或 TAIFEX futures session 的 fallback；
  ...
  `session_profile`；option/warrant reference 與 source identity 納入 EffectiveRunConfig v7 canonical
  identity，partition/cache 路徑與 cache factory 不再自行假設 Teralion；
...
- `ReplayStateViews` 改為借用 replay core 的 deterministic state map，移除一般與 scheduled strategy callback
  每筆 event 配置 market-state view 向量；這是破壞性 API migration，不保留 slice adapter。
...
- TWSE／TPEx 盤中 `trial` 統一輸出 `IntradayUnclassified`，保留方向 annotations 供 Alpha 關聯明確 trigger。真實 `/ticks` stability samples 確認兩市場 zero-quantity／empty-book trigger 形狀；normalizer 將這類 status-only observation 映射為 `MarketStatus`，其他 zero-quantity shape 拒絕。TWSE 同一 `match_time` 多筆 intermediate trades + final trigger 會合併成保序 `TradeBatch` 加 final `MarketStatus`。原始 source rows 未納入 fixtures；Teralion adapter certification 與 Alpha production differential 均已完成。
- Derived cache 單筆 canonical event record 限制 16 MiB；reader 在配置 record buffer 前拒絕超限長度，writer 同步拒絕產生無法讀回的 cache。
